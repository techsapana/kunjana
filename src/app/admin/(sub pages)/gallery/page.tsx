...
export default function AdminGalleryForm() {
  ...
  const handleSubmit = async () => {
    if (!form.title.en.trim() || !form.title.ne.trim()) {
      return alert("Enter gallery title in English and Nepali");
    }
    ...
    try {
      ...
      setForm({ title: emptyText(), images: [] });
      fetchGalleries();
    } catch (err) {
      console.error(err);
      alert("Save failed");
  ...
  const handleEdit = (gallery: Gallery) => {
    setForm({
      id: gallery.id,
      title: resolveLocalizedText(gallery.titleI18n, gallery.title),
      images: [],
    });
  };
    ...
    <div className="min-h-screen bg-white text-black p-6">
      <div className="max-w-5xl mx-auto">
        ...
        <div className="border p-6 rounded-lg shadow-sm mb-8">
          <input
            type="text"
            placeholder="Gallery Title (English)"
            value={form.title.en}
          ...
          <input
            type="text"
            placeholder="ग्यालेरी शीर्षक (नेपाली)"
            value={form.title.ne}
            onChange={(e) => handleTitleChange("ne", e)}
