...
const toApiGallery = (gallery: {
  id: number;
  title: string;
  titleI18n: unknown;
  images: Array<{ id: number; url: string; galleryId: number }>;
  createdAt: Date;
  updatedAt: Date;
}) => {
  const titleI18n = getLocalizedText(gallery.titleI18n, gallery.title);
...
export async function GET() {
  try {
    ...
    return NextResponse.json(galleries.map(toApiGallery));
  } catch (error) {
...
export async function POST(request: Request) {
  try {
    ...
    return NextResponse.json(toApiGallery(created), { status: 201 });
  } catch (error) {
    return NextResponse.json(
      {
        success: false,
...
export async function PUT(request: Request) {
  try {
    ...
    return NextResponse.json(toApiGallery(updated));
  } catch (error) {
    return NextResponse.json(
      {
        success: false,
